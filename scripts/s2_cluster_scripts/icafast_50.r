...
L_icafast_50 <- function(datmat, n_comp = 50, n_rep = 100, max_it = 900, tol_it = 1e-6){
  x <- list()
...
if(data.version == "spikeF") {
  ...
  system.time(out <- L_icafast_50(datmat = exp_mat))
  
  # save on punim - change dir accordingly -----------------#
  saveRDS(out, file = "/data/cephfs/punim0613/icafast/output/ica_spikeF_list_50.rds")
  rm(exp_mat)
...
if(data.version == "sumF") {
  ...
  # save on punim - change dir accordingly -----------------#
  saveRDS(out, file = "/data/cephfs/punim0613/icafast/output/ica_sumF_list_50.rds")
  rm(exp_mat)
